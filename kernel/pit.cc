...
extern "C" void apitHandler(uint32_t* things) {
    ...
    // come in and want to preempt but need to make sure a thread is running currently 
    // why would there be no threads ?? bad inilliazation
    // make sure nopreempt flag is false
    // then skip false if the fals is true
    if ((me != nullptr) && (!me->no_preempt)) {
        yield();
    }
}
